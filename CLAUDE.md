...
- `npm run db:migrate` — apply migrations via `src/db/migrate.ts` (uses `DATABASE_URL`).
...
Env: copy `.env.example` → `.env`. `packages/server/src/config/env.ts` validates env with zod at startup and **exits the process on failure** — when adding a new env var, add it there.
...
`.github/workflows/ci.yml` runs `npm ci → npm run lint → npm run typecheck → npm run build` on push/PR to `main`. Node 20. Keep those three scripts green.
