...
export const users = pgTable(
  ...
  (t) => [
    ...
    pgPolicy("users_tenant_isolation", {
      ...
      for: "select",
      to: "app_user",
      using: sql`${t.organizationId} = NULLIF(current_setting('app.current_tenant', true), '')::uuid OR ${t.id} = NULLIF(current_setting('app.current_user_id', true), '')::uuid`,
    }),
    ...
    pgPolicy("users_update_own", {
      ...
      to: "app_user",
      using: sql`${t.organizationId} = NULLIF(current_setting('app.current_tenant', true), '')::uuid OR ${t.id} = NULLIF(current_setting('app.current_user_id', true), '')::uuid`,
      withCheck: sql`${t.organizationId} = NULLIF(current_setting('app.current_tenant', true), '')::uuid OR ${t.id} = NULLIF(current_setting('app.current_user_id', true), '')::uuid`,
    }),
  ],
);
