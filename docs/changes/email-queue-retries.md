...
**Touches:** `packages/server/src/services/{email-delivery.service,email-error}.ts`, `packages/server/src/queues/{worker,emailBackoff}.ts`, `packages/server/src/ops/probes/backlog.ts`, `packages/server/src/db/schema/emailDeliveries.ts`, `packages/server/drizzle/0014_email_delivery_claim_lease.sql`
