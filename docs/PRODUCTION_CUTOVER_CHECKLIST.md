...
|---|-------|-------------------|----------------|--------|-------|
| 0.1 | Secrets reality check | Grep repo + running container env for `DB_PASSWORD=1234`, `devkey`, `devsecretdevsecret*`. Diff SSM Parameter Store values against `.env` defaults. | Zero default/dev credential strings reachable in any running prod container. | ⚠️ Partial | |
| 0.2 | CI/CD existence | `ls .github/workflows/`, cat `ci.yml`, check branch protection rules in repo settings. | Workflow exists, runs on PR + push to main, and merge is actually blocked on failure (not just present). | ❌ Bug found | |
| 0.3 | RLS enforcement (live test, not migration file review) | Connect as `app_user` role (not superuser) with two different `app.current_tenant` session values; attempt cross-org SELECT on `event_rooms`, `activity_submissions`, `attendance_entries`. | Zero rows returned for wrong org, **and non-zero correct rows returned for right org**, under the actual restricted role. | ❌ **Failed — retracted** | |
| 0.4 | GDPR export/delete — functional test | Call `GET /profile/me/export` and `DELETE /profile/me/account` with a real test token. | Export excludes `passwordHash`; delete sets `deletedAt`/`isActive=false`, clears refresh cookie, and a subsequent login fails. | ⚠️ Partial | |
| 0.5 | Security headers — live, not source | `curl -I` against staging URL. | CSP, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` present on actual responses. | ⚠️ Partial | |
| 0.6 | Dependency & container CVE scan | `npm audit --production`, `osv-scanner` or Snyk on lockfile, `trivy image` on prod Docker images. | No unresolved HIGH/CRITICAL CVEs. | ⚠️ Partial | |
...
- **Two confirmations still needed before applying** (both called out at the top of the SQL file): (1) confirm `app_user`'s actual `rolsuper`/`rolbypassrls` values, (2) confirm your auth service layer can be updated to use `auth_svc_role` for the specific pre-identity queries — if that's more change than wanted right now, a documented weaker fallback is noted inline.
...
- **0.1** — `fetch-secrets.sh` exists, pulls from SSM, validates 16 required keys. **Open:** `deploy.yml`'s SSM `send-command` payload is shown redacted (`commands='[...]'`) — must confirm it actually invokes this script during deploy, and that a missing/weak key fails the *GitHub Actions job*, not just the script in isolation.
- **0.2 — CRITICAL BUG:** `deploy.yml` triggers on `workflow_run: workflows: ["CI"]`, but `ci.yml` is named `Continuous Integration`. `workflow_run` matches on the workflow's `name:` field exactly — this mismatch means **deploy.yml will never fire automatically after CI passes.** Fix: rename one to match the other. Also open: `npm audit` runs with `continue-on-error: true` (won't block on HIGH/CRITICAL); CI uses Node 24 while Dockerfiles use `node:22-alpine` (align them); branch protection ("required status check") must be confirmed in repo settings, not assumed from YAML; the note "E2E tests will passs" is unclear — confirm whether it means tests pass, or pass without exercising a real running app (check Playwright `webServer` config).
- **0.3** — Cross-org test passed under `test_app_user`. **Open:** confirm the *production* app's DB pool connects as this restricted role, not the table owner — owners bypass RLS by default unless `FORCE ROW LEVEL SECURITY` is explicitly set.
- **0.4** — Endpoints implemented correctly (redacts `passwordHash` + `twoFactorSecret`, audit-logs both actions). **Open:** report explicitly states no automated tests exist yet for either endpoint — add integration tests before treating this as verified, given it's the GDPR compliance surface.
...
- **0.6** — Server image: 0 HIGH/CRITICAL (clean). **Open:** client (nginx) image has 104 HIGH/CRITICAL (11 CRITICAL), all Debian 12 OS packages, not app code. Needs an explicit decision: swap to `nginx:1.29-alpine` to shrink the surface, or accept with a documented monthly base-image patch cadence.
...
- **0.9** — Not addressed this round. Still needs a decision on whether `sessions`/`password_resets`/`email_verifications` need RLS at all (they scope by user, not org) or an alternative access-control check.
