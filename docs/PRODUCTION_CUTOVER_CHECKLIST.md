...
| 0.3 | RLS enforcement (live test, not migration file review) | Connect as `app_user` role (not superuser) with two different `app.current_tenant` session values; attempt cross-org SELECT on `event_rooms`, `activity_submissions`, `attendance_entries`. | Zero rows returned for wrong org, **and non-zero correct rows returned for right org**, under the actual restricted role. | ❌ Release blocker — policies exist and work under `app_user`, but production `DATABASE_URL` connects as superuser `Eventclick_admin` which bypasses RLS entirely; `FORCE ROW LEVEL SECURITY` is not set on any table | |
...
### Round 2 Findings — RLS exists but production connection bypasses it (release-blocking)

- **Current DB state (verified via `pg_policies`):** 13 `tenant_isolation` policies exist across tenant tables (`event_rooms`, `activity_submissions`, `attendance_entries`, `audit_logs`, `users`, `organizations`, `activity_photos`, `event_admin_assignments`, `form_definitions`, `room_recordings`, `notifications`, `org_members`, `feedback`, `bug_reports`, `pdf_jobs`). RLS is enabled on 6 tenant tables.
- **Live test as `app_user` (verified):**
...
- **Release-blocking finding:** All tables have `relforcerowsecurity = false`. The current `DATABASE_URL` in `.env` connects as `Eventclick_admin`, which is a superuser (`rolsuper = true`). Superusers bypass RLS entirely regardless of policies. **Production must switch `DATABASE_URL` to connect as `app_user` (or enable `FORCE ROW LEVEL SECURITY`) for tenant isolation to actually protect data.**
- **Remaining unprotected tables:** `sessions`, `password_resets`, `email_verifications` have no RLS and no policies. This is an accepted design decision — these tables are user-scoped, not org-scoped, and access is controlled at the application layer. See 0.9.
...
- **0.3** — Live test passed as `app_user` (verified: `rolsuper=false`, `rolbypassrls=false`). 13 `tenant_isolation` policies exist and enforce `organization_id = current_setting('app.current_tenant')`. **Critical finding:** current `DATABASE_URL` connects as `Eventclick_admin` (superuser), which bypasses all RLS because `relforcerowsecurity = false` on every table. **Release blocker:** production must switch the app DB user to `app_user` or enable `FORCE ROW LEVEL SECURITY`. Code prep complete: `authDb`/`authPool` wired for pre-tenant auth queries; `0022_add_missing_rls_policies.sql` already reflected in live DB. `sessions`/`password_resets`/`email_verifications` intentionally left without RLS — see 0.9.
