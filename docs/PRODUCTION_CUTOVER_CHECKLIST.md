...
| 0.3 | RLS enforcement (live test, not migration file review) | Connect as `app_user` role (not superuser) with two different `app.current_tenant` session values; attempt cross-org SELECT on `event_rooms`, `activity_submissions`, `attendance_entries`. | Zero rows returned for wrong org, **and non-zero correct rows returned for right org**, under the actual restricted role. | ⚠️ Partial — code ready, pending DB apply | |
